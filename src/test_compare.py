...
def get_unique_count(nodes, key_fields):
    ...
    for node in nodes:
        ...
        key = "_".join(str(node.attrib.get(field, "")) for field in key_fields)
...
for key in cb_root.keys():
    ...
    root_info.append({"Property": key, "CB": cb_root.get(key), "AC": ac_root.get(key)})
...
for config in NODE_CONFIG:
    ...
    differences, zero_values, missing_records, duplicate_records = compare_nodes(
        cb_nodes, ac_nodes, node_name, config["display_path"], config["key_fields"]
    )
    ...
    total_issues = len(differences) + len(missing_records) + len(duplicate_records)

    cb_count = get_unique_count(cb_nodes, config["key_fields"])

    ac_count = get_unique_count(ac_nodes, config["key_fields"])
...
print("Total Missing Records:", len(all_missing_records))

print("Total Zero Values:", len(all_zero_values))

print("Total Duplicates:", len(all_duplicate_records))
